...
これは論文からの引用です。VAE は、損失関数 Lvae(x)  =  Dave(x) + Avae(x) + Mvae(x) と３つの項から構成されています。１つ目 Dvae(x) は、与えられたデータの中から頻度の高い特徴を学習し、頻度の少ない特徴を無視する性質を持つ項。２つ目 Avae(x) は、与えられたデータの中の特徴の複雑さに応じて調整する性質を持つ項。そして３つ目 Mvae(x) は、直接的に再現誤差に関係している項です。
